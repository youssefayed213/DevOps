...
class ChambreServiceTest {
    ...
    void testGetChambresParNomBloc() {
        // Arrange
        Chambre chambre = new Chambre();
        when(chambreRepository.findByBlocNomBloc(TEST_BLOC_NAME)).thenReturn(Arrays.asList(chambre));

        // Act
        List<Chambre> result = chambreService.getChambresParNomBloc(TEST_BLOC_NAME);

        // Assert
        assertNotNull(result);
        assertEquals(1, result.size());
        verify(chambreRepository, times(1)).findByBlocNomBloc(TEST_BLOC_NAME);
    }
    ...
    @Test
    void testGetChambresNonReserveParNomFoyerEtTypeChambreEmptyResult() {
